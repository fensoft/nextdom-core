...
nextdom.history.drawChart = function (_params) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            _params.option.graphColor = (isset(nextdom.history.chart[_params.el])) ? init(_params.option.graphColor, Highcharts.getOptions().colors[init(nextdom.history.chart[_params.el].color, 0)]) : init(_params.option.graphColor, Highcharts.getOptions().colors[0]);
            ...
            if(isset(_params.newGraph) && _params.newGraph == true){
                delete nextdom.history.chart[_params.el];
            }
            var charts = {
                zoomType: 'x',
                renderTo: _params.el,
            ...
            if( _params.option.graphType == 'pie'){
                ...
                if (!isset(nextdom.history.chart[_params.el]) || (isset(_params.newGraph) && _params.newGraph == true)) {
                    ...
                    nextdom.history.chart[_params.el].chart = new Highcharts.Chart({
                        chart: charts,
                        title: {
                            text: ''
                        },
                    ...
                    nextdom.history.chart[_params.el].chart.series[0].addPoint({y:data.result.data[data.result.data.length - 1][1], name : (isset(_params.option.name)) ? _params.option.name + ' '+ data.result.unite : data.result.history_name + ' '+ data.result.unite, color: _params.option.graphColor});
                }
            }else{
                var dataGrouping = {
                    enabled: false
                ...
                if(data.result.timelineOnly){
                    if(!isset(nextdom.history.chart[_params.el]) || !isset(nextdom.history.chart[_params.el].nbTimeline)){
                        nbTimeline = 1;
                    }else{
                        nextdom.history.chart[_params.el].nbTimeline++;
                        nbTimeline = nextdom.history.chart[_params.el].nbTimeline;
                    }
                    var series = {
                        type: 'flags',
                ...
                if (!isset(nextdom.history.chart[_params.el]) || (isset(_params.newGraph) && _params.newGraph == true)) {
                    ...
                    nextdom.history.chart[_params.el].chart.addSeries(series);
                }
            ...
            for (var i in nextdom.history.chart) {
                ...
                if (serie != null && serie != undefined) {
                    ...
                    nextdom.history.drawChart({
                        ...
                        el: i,
                        dateRange: nextdom.history.chart[i].cmd[_params.cmd_id].dateRange,
                        option: nextdom.history.chart[i].cmd[_params.cmd_id].option
                    });
                }
            }
        }

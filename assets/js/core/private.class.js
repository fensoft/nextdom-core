...
nextdom.private.getParamsAJAX = function(_params, target, action) {
    ...
    var paramsAJAX = {
        ...
        error: function(_request, _status, _error) {
            _params.error(nextdom.private.handleAjaxErrorAPI(_request, _status, _error));
        },
...
nextdom.private.checkParamValue = function(_params) {
    try {
 ...
 */
nextdom.private.checkParamsRequired = function(_params, _paramsRequired) {
    var missings = Array();
    var group = Array();
 ...
 */
nextdom.private.checkAndGetParams = function(_params, _paramsSpecifics, _paramsRequired) {
    // throw une exception en cas d'erreur (à attraper plus haut)
    nextdom.private.checkParamsRequired(_params, _paramsRequired || []);

    // tout est ok, on merge avec les paramètres par défaut + les spécifiques à la fonction
    var params = $.extend({}, nextdom.private.default_params, _paramsSpecifics, _params || {});
 ...
 */
nextdom.private.checkParamsValue = function(_params) {
    if (Object.prototype.toString.call(_params) == '[object Object]') {
        nextdom.private.checkParamValue(_params);
    } else {
        for (var i in _params) {
            nextdom.private.checkParamValue(_params[i]);
        }
    }
};